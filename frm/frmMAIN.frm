...
Begin VB.Form frmMAIN 
   ...
   Begin VB.CheckBox chMakeVFrames 
      ...
      Left            =   10560
      TabIndex        =   27
      ToolTipText     =   "If there's a new Best Creature at every 25 Generation, draw frames (good at 30fps)"
      Top             =   5160
      Width           =   1335
   End
   ...
   Begin VB.PictureBox PicGR 
      ...
      ScaleWidth      =   561
      TabIndex        =   14
      ToolTipText     =   "Green=Best Creature   Blue=AVG Fitness   Cyan=Social Diversity  Red=Mutation Prob."
      Top             =   1320
      Width           =   8415
   End
    ...
    For GEN = 1 To 99999999
        ...
        Do
            ...
            If CNT Mod CyclesForScreenFrame = 0 Then
                If DoFrames Then
                    ...
                    AAA.LineDIB 0, MaxY \ 1, Me.ScaleWidth, MaxY \ 1, vbBlack
                    ...
                    For I = 1 To NC
                        If GA.IsIndiChanged(I) Then
                            CRE(I).DRAW frmMAIN.hdc, CamX \ 1, chDrawPts, True
                        End If
                    Next
                ...
                Else
                    ...
                    FastLine Me.hdc, 0, MaxY \ 1, Me.ScaleWidth, MaxY \ 1, 1, vbWhite
                    For X = 0 To Me.ScaleWidth * 3 Step 100
                        XmCAMX = X - CamX
                ...
                If chDisplayInputs Then
                    ...
                    CY = DrawInputsY + 40
                    CX = DrawInputsX - 180
                    FastLine Me.hdc, CX, CY, CX + (RotIdx - PTSidx) * 4, CY, 1, vbRed
                    ...
                    FastLine Me.hdc, CX, CY + 20, CX + 100, CY + 20, 1, vbGreen
                    For I = 1 To Nout
                        CX = (DrawInputsX + 225) + (I - 1) * 100 / (Nout - 1)
                ...
                MyCircle Me.hdc, CRE(CurBEST).CurrCenX - CamX + CRE(CurBEST).DrawOffset, CRE(CurBEST).CurrCenY, 2, 3, vbRed
                ...
                If CNT Mod CyclesForScreenFrame = 0 Then
                    ...
                    For I = 1 To NC
                        If GA.IsIndiChanged(I) Then
                            ...
                            FFF = FFF + sDontJump * CRE(I).TaskDontJUMP / TrainTime
                            FFF = FFF + sAVGH * CRE(I).TaskAVGheight / TrainTime
                            FFF = FFF + sMaxH * CRE(I).TaskMAXHeight
                            FFF = FFF + sVertical * CRE(I).TaskVerticality / TrainTime
                            FFF = FFF + sHorizontal * CRE(I).TaskHorizontality / TrainTime
                            If FFF < CurBESTV Then CurBESTV = FFF: CurBEST = I: PanX = CRE(I).CurrCenX - Me.ScaleWidth * 0.5 + CRE(I).DrawOffset
            ...
            If CNT Mod 500 = 0 Then Me.Caption = "Generation:" & GEN & "  " & CNT & "    (Avg" & GA.STAT_AVGfit & ")" & "    (Best" & GA.STAT_GenerBestFit & ")    (Video: " & Frame \ 30 & " Seconds at 30fps)": If CNT = TrainTime Then Exit Do
        ...
        For I = 1 To NC
            If GA.IsIndiChanged(I) Then
                ...
                GA.IndiFitness(I) = GA.IndiFitness(I) + sDontJump * CRE(I).TaskDontJUMP / TrainTime
                GA.IndiFitness(I) = GA.IndiFitness(I) + sAVGH * CRE(I).TaskAVGheight / TrainTime
                GA.IndiFitness(I) = GA.IndiFitness(I) + sMaxH * CRE(I).TaskMAXHeight
                GA.IndiFitness(I) = GA.IndiFitness(I) + sVertical * CRE(I).TaskVerticality / TrainTime
                GA.IndiFitness(I) = GA.IndiFitness(I) + sHorizontal * CRE(I).TaskHorizontality / TrainTime
            End If
        Next
        ...
        'If chMakeVFrames Then If GEN = 2 Or (GEN Mod 25 = 0) Then DoFrames = True
        'if chMakeVFrames Then If GEN = 1 Or (GEN Mod 25) = 0 Then If DoVideoBEST <> GA.STAT_GenerBestFitINDX Then DoFrames = True: DoVideoBEST = GA.STAT_GenerBestFitINDX: I = GA.STAT_GenerBestFitINDX: GA.IsIndiChanged(I) = True
        If chMakeVFrames Then If GEN = 1 Or ((GEN + 1) Mod 25) = 0 Then If DoVideoBEST <> GA.STAT_GenerBestFitINDX Then DoFrames = True: DoVideoBEST = GA.STAT_GenerBestFitINDX: GA.IsIndiChanged(DoVideoBEST) = True:
...
Private Sub INITIALIZENN()
    ...
    Ninp = (CRE(1).NL - Nout) * 2 + CRE(1).NP + 4 + 1 + 2 + 2 + 2
    'Ninp = (Nout) * 2 + CRE(1).NP + 4 + 1 + 2 + 2
    'Nout = Nout * 2
    ...
    S = S & "Links : " & CRE(1).NL & " (with Brain: " & Nout & ". Without Brain: " & CRE(1).NL - Nout & ")" & vbCrLf & vbCrLf
    ...
    BR.InitBrains NC, 1
    BR.InitBrainCell 1, Array(Ninp, Nout * 1.5, Nout), 5, PIC    '5

    'MutateOnlySons, 0.05, 0.2, 0.15,
    'sns x gen .12 --> 3
    GA.INIT1_EvolutionParams MutateOnlySons, 0.01, 0.5, 0.05, 0.1, SelWheelSQ, CrossBySwap, SonToWorst, False, 20
